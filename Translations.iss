...
jp.MsgIncompatibleModsDetected=警告 - あなたのゲームフォルダで互換性のない改造が検出されました！ あなたは非常に古くなったモッズを使っているか、ＡＩ*少女がの他のゲームのモッズを使っています。 これを修正するには、古いモッズをすべて削除する必要があります。
jp.MsgExeNotFound=警告 - ＡＩ*少女が選択されたディレクトリに見つかりませんでした。 正しく動作させるためには、このパッチをＡＩ*少女がのメインゲームディレクトリに直接インストールする必要があります。%n%nこのディレクトリが正しいことを確認していますか？
...
jp.TaskDeletePlugins=古いプラグインとスクリプトを削除する（古いプラグインを削除します。問題がある場合や古いバージョンを更新する場合に推奨）
